...
class AddBookDetailsByTitle extends StatefulWidget {
  AddBookDetailsByTitle({super.key, required this.title});
  final String title;

  @override
  State<AddBookDetailsByTitle> createState() => _AddBookDetailsByTitleState();
}

class _AddBookDetailsByTitleState extends State<AddBookDetailsByTitle> {
  Future<Map>? _bookData;
  double? _deviceWidth;
  @override
  void initState() {
    super.initState();
    _bookData = getBookDataByTitle(widget.title);
  }
        ...
        child: FutureBuilder(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              double avgRating = (_data?['avgRating'] ?? 0).toDouble();
              List categories = _data?['categories'] ?? ['No Categories'];
  ...
  Widget _averageRating(double avgRating) {
    return Text(
  ...
  Widget _genres(List categories) {
    return Text(
